...
path = "/home/olle/PycharmProjects/derive_depth_vector_and_map/grid_not_calculates_file_dirs"
study_dates = np.loadtxt(path, dtype='str')
        ...
        for folders in study_folders:
            if "Volume" in folders:
                ...
                for study_file in study_files:
                        ...
                        for Laterality in Lateralities:
                            ...
                            if OCT_pos_pixel["Laterality"].isin([Laterality]).any():
                                ...
                                for fundus_time in fundus_times:
                                    x_cord, y_cord, x_start, y_start, x_end, \
                                    y_end, y_indices, x_indices = xir.get_xml_indices(Laterality,OCT_pos_pixel,
                                                                                      fundus_time, im_pd)
                                    ...
                                    oct_times_in_folder_and_xml = oct_times_pd[pd.Series(oct_times)\
                                        .isin(im_pd[im_pd.series == series_id].Image_aq_time)]
                                    ...
                                    for oct_time in oct_times_in_folder_and_xml:
                                        ...
                                        data_loading_time.append(abs((end_data_loading.microsecond - start_data_loading.microsecond)) / 1e6)
                                    ...
                                    prediction_time.append(abs((end_prediction.microsecond - start_prediction.microsecond)) / 1e6)
                                    pred_array = np.asarray(predictions).reshape(len(oct_slice), img_height, img_width).astype(np.int32)
                                    #print(pred_array.shape[0])
                                    # reshape the segmentations to original shape
                                    im_batch = []
                                    # resize all images
                                    for i in range(0, pred_array.shape[0]):
                                        im_resized = np.asarray(Image.fromarray(pred_array[i]).resize([int(new_shape[1]), int(new_shape[0])]))
                                    ...
                                    for i in range(0, im_batch.shape[0]):
                                        ...
                                        try:
                                            if y_cord == "iterable":
                                                ...
                                                if Laterality == "L":
                                                    grid[int(y_indices["starty_pos"].values[im_id-1]), x_start:x_end] = d_v
                                                if Laterality == "R":
                                                    grid[int(y_indices["starty_pos"].values[im_id-1]), x_start:x_end] = d_v
                                            if x_cord == "iterable":
                                                # assert indices are ints
                                                ...
                                                # shift indices when laterilty changes to "L"
                                                if Laterality == "L":
                                                    grid[y_start:y_end, int(x_indices["startx_pos"].values[im_id - 1])] = d_v
                                                if Laterality == "R":
                                                    grid[y_start:y_end, int(x_indices["startx_pos"].values[im_id-1])] = d_v
                                        except:
                    ...
                    patient_time.append(abs((end_data_loading_patient.microsecond - starting_data_loading_patient.microsecond)) / 1e6)

                    format_str = ('Total processing time per patient is %.2f, data load = %.2f, prediction time = %.2f, study_time = %.2f')
                    # take averages of all the accuracies from the previous bathces
                    print(format_str % (np.mean(patient_time), np.mean(data_loading_time), np.mean(prediction_time), np.mean(study_time)))
